...
.left-column[
.center[
![:scale 100%](../images/buchanantullock2.png)

.smallest[
L: James Buchanan (1919—2013)

R: Gordon Tullock (1922—2014)
]
]
]
...
.center[
<iframe width="980" height="550" src="https://www.youtube.com/embed/5u8vd_YNbTw" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>]
...
.pull-left[
- People often recommend optimal policies that could effectively be installed by a .hi[benevolent despot] who has
  - Perfect knowledge to correctly identify ‘the public good’
  - Has the desire and perfect incentives to achieve it
  - Has no constraints on his/her actions
]
...
- In reality, 1<sup>st</sup>-best policies distorted by:
  - problems with knowledge
  - problems with incentives
  - problems with politics/agreement/control
...
> “The view that now pervades much public policy economics implicitly presents the relevant choice as between an ideal norm and an existing 'imperfect' institutional arrangement. This .hi[nirvana approach] differs considerably from a comparative institution approach in which the relevant choice is between alternative real institutional arrangements,” (p.1).
...
- Compare imperfections of feasible and relevant alternative systems
    - .hi[Nirvana fallacy]: comparing a flawed (but real) system with some flawless, ideal system
...
> “We are, therefore, placed in the somewhat singular position of having to defend the simple assumption that .hi[the *same* individual participates in both processes [the market and politics]] against the almost certain onslaught of the moralists,” (p. 19)
...
- .hi-purple[What _further_ collective action will individuals agree to?]
...
ggplot(data.frame(x=c(1,100)), aes(x=x))+
  ...
  geom_label(x=70, y=C(70), label="C(n)", color = "red")+
...
.pull-right[
- .hi-red[External costs, `\\(C(n)\\)`]: cost of collectively-chosen outcome on individual, falling with $n$
...
ggplot(data.frame(x=c(1,100)), aes(x=x))+
  ...
  geom_label(x=70, y=D(70), label="D(n)", color = "blue")+
  labs(x = "Fraction of N Individuals Needed",
       y = "Costs")+
  scale_x_continuous(breaks = c(0,100),
                     labels = c(0, "N"),
...
.pull-right[
- .hi-blue[Decision-making costs, `\\(D(n)\\)`]: cost of reaching an agreement, rising at an increasing rate with $n$
...
ggplot(data.frame(x=c(1,100)), aes(x=x))+
  ...
  geom_label(x=70, y=Total(70), label="C(n)+D(n)", color = "purple")+
  labs(x = "Fraction of N Individuals Needed",
       y = "Costs")+
  scale_x_continuous(breaks = c(0,100),
                     labels = c(0, "N"),
...
opt<-ggplot(data.frame(x=c(1,100)), aes(x=x))+
  ...
  stat_function(fun=Total, geom="line",color="purple", size =2)+
  geom_label(x=70, y=Total(70), label="C(n)+D(n)", color = "purple")+
  geom_segment(x=min, xend=min, y=0, yend=Total(min), color="green",linetype="dashed", size=1.5)+
  labs(x = "Fraction of N Individuals Needed",
       y = "Costs")+
...
- Note: again, the **minimum of** .hi-purple[Total Costs]
  ...
  - not necessarily **simple majority $\frac{n}{2}+1$**!
]
...
- High .hi-red[`\\(C(n)\\)`]-issues
  ...
  - .hi-purple[Require much larger decisive set than a simple majority]
...
ggplot(data.frame(x=c(1,100)), aes(x=x))+
  stat_function(fun=C_2, geom="line",color="red", size =2)+
  geom_label(x=90, y=C_2(90), label="C(n)", color = "red")+
  stat_function(fun=D, geom="line",color="blue", size =2)+
  ...
  geom_segment(x=min_2, xend=min_2, y=0, yend=Total_2(min_2), color="green",linetype="dashed", size=1.5)+
  labs(x = "Fraction of N Individuals Needed",
       y = "Costs")+
...
- Low .hi-red[`\\(C(n)\\)`]-issues, common access rules:
  ...
  - *Writ of certorari*
  - .hi-purple[Require much smaller decisive set than a simple majority]
]
...
C_3=function(x){a_3*exp(-x/c)}
D=function(x){b*exp(x/d)}
#total costs function is sum of C and D functions
Total_3=function(x){C_3(x)+D(x)}

minimum3<-optimize(Total_3, interval=c(0, 100), maximum=FALSE)

min_3<-round(minimum3$minimum,0)

ggplot(data.frame(x=c(1,100)), aes(x=x))+
  stat_function(fun=C_3, geom="line",color="red", size =2)+
  geom_label(x=90, y=C_3(90), label="C(n)", color = "red")+
  stat_function(fun=D, geom="line",color="blue", size =2)+
  geom_label(x=60, y=D(60), label="D(n)", color = "blue")+
  stat_function(fun=Total_3, geom="line",color="purple", size =2)+
  geom_label(x=60, y=Total_3(60), label="C(n)+D(n)", color = "purple")+
  geom_segment(x=min_3, xend=min_3, y=0, yend=Total_3(min_3), color="green",linetype="dashed", size=1.5)+
  labs(x = "Fraction of N Individuals Needed",
       y = "Costs")+
  scale_x_continuous(breaks = c(0,min_3,100),
                     labels = c(0,"K/N","N"),
                     limits = c(0,100),
                     expand = c(0,0))+
  scale_y_continuous(breaks = NULL,
...
.center[
![:scale 60%](../images/ccmodel.png)

[https://ryansafner.shinyapps.io/ccmodel/](https://ryansafner.shinyapps.io/ccmodel/)
]
...
- Size of $N$: larger groups create larger $C+D$ costs
  - .hi-purple[We want the smallest group consistent with the size of the externality]
...
ggplot(data.frame(x=c(1,100)), aes(x=x))+
  stat_function(fun=Total, geom="line",color="purple", size =2)+
  geom_label(x=70, y=Total(70), label="C(n)+D(n)", color = "purple")+
  geom_segment(x=min, xend = min, y = 0, yend = min_level, color="green", linetype="dashed", size=1.5)+
